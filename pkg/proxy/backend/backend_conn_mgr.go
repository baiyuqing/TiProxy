...
func NewBackendConnManager(
    logger *zap.Logger, handshakeHandler HandshakeHandler, connectionID uint64, config *BCConfig,
) *BackendConnManager {
    config.check()
...
func (mgr *BackendConnManager) Connect(
    ctx context.Context, clientIO *pnet.PacketIO, frontendTLSConfig, backendTLSConfig *tls.Config,
) error {
    ...
    err := mgr.authenticator.handshakeFirstTime(
        mgr.logger.Named("authenticator"), mgr, clientIO, mgr.handshakeHandler, mgr.getBackendIO, frontendTLSConfig,
        backendTLSConfig,
	)
...
func (mgr *BackendConnManager) getBackendIO(
    cctx ConnContext, auth *Authenticator, resp *pnet.HandshakeResp, timeout time.Duration,
) (*pnet.PacketIO, error) {
    ...
    if err != nil {
        ...
        mgr.logger.Warn(
            "get backend slow", zap.Duration("duration", duration), zap.NamedError("last_err", origErr),
...
func (mgr *BackendConnManager) tryRedirect(ctx context.Context) {
    ...
    if rs.err = mgr.authenticator.handshakeSecondTime(
        mgr.logger, mgr.clientIO, newBackendIO, mgr.backendTLS, sessionToken,
	); rs.err == nil {
...
func (mgr *BackendConnManager) notifyRedirectResult(ctx context.Context, rs *redirectResult) {
    ...
    if rs.err != nil {
        ...
        mgr.logger.Warn(
            "redirect connection failed", zap.String("from", rs.from),
        ...
        mgr.logger.Info(
            "redirect connection succeeds", zap.String("from", rs.from),
...
func (mgr *BackendConnManager) tryGracefulClose(ctx context.Context) {
    ...
    if err := mgr.clientIO.GracefulClose(); err != nil {
        mgr.logger.Warn(
            "graceful close client IO error", zap.Stringer("addr", mgr.clientIO.RemoteAddr()), zap.Error(err),
		)
    }
...
func (mgr *BackendConnManager) checkBackendActive() {
    ...
    if !mgr.backendIO.IsPeerActive() {
        mgr.logger.Info(
            "backend connection is closed, close client connection", zap.Stringer("client", mgr.clientIO.RemoteAddr()),
